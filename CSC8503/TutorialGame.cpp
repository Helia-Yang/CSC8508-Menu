...
void TutorialGame::UpdateGame(float dt) {
	...
	if (Window::GetKeyboard()->KeyPressed(KeyboardKeys::K) && selectionObject) {
		...
		if (world->Raycast(r, closestCollision, true, selectionObject)) {
			if (objClosest) {
				...
				Debug::DrawLine(selectionObject->GetTransform().GetPosition(), objClosest->GetTransform().GetPosition(), Vector4(1, 0, 0, 1), 0.0f);
			}
	...
	if (testStateObject) {
		...
		for (int i = 0; i < nodes.size() - 1; ++i) {
			Debug::DrawLine(nodes[i], nodes[i + 1], Vector4(0, 1, 0, 1), 0.0f);
		}
	}
	if (goose) {
		goose->Update(dt);
		for (int i = 0; i < nodes2.size() - 1; ++i) {
			Debug::DrawLine(nodes2[i], nodes2[i + 1], Vector4(0, 1, 0, 1), 0.0f);
		}
	}
